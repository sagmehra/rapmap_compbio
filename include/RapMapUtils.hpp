...
namespace rapmap {
    ...
    struct QuasiAlignment {
  	QuasiAlignment() :
        ...
        // this cigar sting is passed to writeAlignmentsToStream for final SAM format output
        std::string cigarString;
    };
    ...
    struct ProcessedSAHit {
        // TBD: how to add cigarString if default constructor is called
	    ...
	    ProcessedSAHit(uint32_t txpIDIn, uint32_t txpPosIn, uint32_t queryPosIn, bool queryRCIn, std::string cigarString = ""):
		    tid(txpIDIn), active(false), numActive(1), cigarStr(cigarString)
	    {
